...
public class NameRepresentationSufixTest {
    ...
    public void testShouldGivePersonsFromSpecificCountry() throws Exception {
        ...
        NameRepresentationSufix representaion = new NameRepresentationSufix();
        ArrayList <Person> selectedList = n.getListBy(personsList,"India");
        Iterator iterator = selectedList.iterator();
        assertEquals("Ms Metla, RadhaKrishna, India",representaion.getNameWithCountry((Person) iterator.next()));
        assertEquals("Mr Gonne, Harsha, India",representaion.getNameWithCountry((Person) iterator.next()));
        try{
